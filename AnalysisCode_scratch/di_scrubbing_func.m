function [data_tab, M_out, OPGEE_bin] = di_scrubbing_func(M_raw, Basin_Select, Basin_Index, activityfolder)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% This function:
% (i)   takes raw drillinginfo data and organizes into categories 
%       according to a specified GOR cutoff 
% (ii)  Bins raw drillinginfo data according to productivity
%
% Inputs:
%   M_raw:          Raw drilling info data (filtered for basin in "tranche_gen_func"
...
%
% Outputs:
%   data_tab:       Matrix of well count and oil/gas production totals for
%                   basin
%   M_out:          Raw drillinginfo data is separated into categories and
%                   units are converted
%                        Col 1 = oil (bbl/day)
%                        Col 2 = gas (Mscf/day)
%                        Col 3 = GOR (Mscf/bbl)
%                   Categories according to GOR cutoff (specified below)
%                       Dry gas from gas wells
%                       Gas with associated oil
%                       Oil only
%                       Oil with gas
%   OPGEE_bin:      Drillinginfo data binned by gas per well productivity
%                       Col 1 = well count
%                       Col 2 = Average gas productivity (mscf/well/day)
%                       Col 3 = Total gas production (mscf/day)
%                       Col 4 = Total oil production (bbl/day)
%
% This function calls one subfunctions:
% data_class:       takes raw drillinginfo data and organizes into categories 
%                   according to a specified GOR cutoff 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[M_out,count,totalprod,averageprod] = data_class(M_new(:,1:2), cutoff, activityfolder);
...
%(i) Determine bin indices and bin means
    ...
    [counts_gas, edges_gas, ind_gas] = histcounts(M_out.drygas(:,2), edges_gas_set);
    
%(ii) Determine bin means
    bin_ave_gas = accumarray(ind_gas, M_out.drygas(:,2),[],@mean);
    bin_sum_gas = accumarray(ind_gas, M_out.drygas(:,2),[],@sum);
    bin_sum_oilwg = accumarray(ind_gas, M_out.drygas(:,1),[],@sum);
...
%(i) Determine bin indices and bin means
    edges_gas_set = [0, 1, 5, 10, 20, 50, 100, 500, 1000, 10000,500000000];
    [counts_gas, edges_gas, ind_gas] = histcounts(M_out.gaswoil(:,2), edges_gas_set);
    
%(ii) Determine bin means
    bin_ave_gas = accumarray(ind_gas, M_out.gaswoil(:,2),[],@mean);
    bin_sum_gas = accumarray(ind_gas, M_out.gaswoil(:,2),[],@sum);
    bin_sum_oilwg = accumarray(ind_gas, M_out.gaswoil(:,1),[],@sum);
...
%(ii) Determine bin indices and bin means
    ...
    [counts_oil, edges_oil, ind_oil] = histcounts(M_out.oilwgas(:,2), edges_oil_set);
    
% %(iii) Determine bin means

    bin_ave_gas = accumarray(ind_oil, M_out.oilwgas(:,2),[],@mean);
    bin_sum_gas = accumarray(ind_oil, M_out.oilwgas(:,2),[],@sum);
    bin_sum_oilwg = accumarray(ind_oil, M_out.oilwgas(:,1),[],@sum);
...
%(ii) Determine bin indices and bin means
    ...
    [counts_oil, edges_oil, ind_oil] = histcounts(M_out.oil(:,2), edges_oil_set);
    
% %(iii) Determine bin means

    bin_ave_gas = accumarray(ind_oil, M_out.oil(:,2),[],@mean);
    bin_sum_gas = accumarray(ind_oil, M_out.oil(:,2),[],@sum);
    bin_sum_oilwg = accumarray(ind_oil, M_out.oil(:,1),[],@sum);
...
%(ii) Determine bin indices and bin means
    ...
    M_all = [M_out.oilall; M_out.gasall];
