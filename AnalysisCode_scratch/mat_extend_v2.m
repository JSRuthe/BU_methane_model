...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% This function generates emissions totals for printing
%
% Important note: Extending the autorun outputs is no longer necessary
% since we are running autorun on a well-by-well basis and not binning by
% tranche (see commented code - will delete at a later edit)
%
% Output data:
%   Emissions Gas and EmissionsOil which are printed as outputs:
%              gas  oil  dataraw	
%           row 1  (18) (6) - Wells
%           row 2  (19) (7) - Header
%           row 3  (20) (8) - Heater
...
%     dataplot.gaswoil = dataplot.gas((dataplot.gas(:,1) > 30 & data(:,1) < 61),:);
    dataplot.drygas = dataplot.gas((dataplot.gas(:,1) < 11),:);
    dataplot.gaswoil = dataplot.gas((dataplot.gas(:,1) > 10 & dataplot.gas(:,1) < 21),:);
...
for i = 1:17
    ...
    if ismember(index,gas_rows)
        counter = counter + 1;
        equip_emissions = dataplot.gas(:,gas_rows(counter));
...
for i = 1:17
    ...
    if ismember(index,oil_rows)
        counter = counter + 1;
        equip_emissions = dataplot.oilall(:,oil_rows(counter));
        equip_emissions(equip_emissions==0) = NaN;
        ind = isnan(equip_emissions);
        equip_emissions = equip_emissions(~ind);
        if ~isempty(equip_emissions)
