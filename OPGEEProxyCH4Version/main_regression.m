...
%% Load methane model results

% LFR = loss rates 
% AFR = activity factor ratios

load('Mat_LFR_22.7.31.mat')
load('Mat_AFR_22.7.31.mat')
sampling_AF1.gas = sampling_AF.gas;
sampling_AF1.oil = sampling_AF.oil;
Mat_LHS1.Gas = Mat_LHS.Gas;
Mat_LHS1.Oil = Mat_LHS.Oil;
load('Mat_LFR_22.7.31_2.mat')
load('Mat_AFR_22.7.31_2.mat')
sampling_AF2.gas = sampling_AF.gas;
sampling_AF2.oil = sampling_AF.oil;
Mat_LHS2.Gas = Mat_LHS.Gas;
Mat_LHS2.Oil = Mat_LHS.Oil;

load('Mat_base_22.7.30.mat')

sampling_AF.gas = cat(1,sampling_AF1.gas,sampling_AF2.gas);
sampling_AF.oil = cat(1,sampling_AF1.oil,sampling_AF2.oil);
Mat_LHS.Oil = cat(3,Mat_LHS1.Oil, Mat_LHS2.Oil);
Mat_LHS.Gas = cat(3,Mat_LHS1.Gas, Mat_LHS2.Gas);
...
if prod_index == 1
    ...
    % gasreduced -> refers to reducing tranche count from 10 to 3
    ...
    % This matrix is used for plotting only
    ...
    for i = 1:9
        ...
        for j = 1:3
            ...
            if j == 1
                ...
                X_trainset = repmat(sampling_AF.gas(idxTrain,:),3,1);
                X_predset = repmat(sampling_AF.gas(idxNew,:),3,1);
      
                Y_trainset = Mat_LFR_gasreduced.set1(i,:)';
                Y_predset = cat(1,squeeze(Mat_LFR.Gas(i,1,idxNew))...
            ...
            elseif j == 2
                ...
                X_trainset = repmat(sampling_AF.gas(idxTrain,:),4,1); 
                X_predset = repmat(sampling_AF.gas(idxNew,:),4,1);
                                           
                Y_trainset = Mat_LFR_gasreduced.set2(i,:)';
               Y_predset = cat(1,squeeze(Mat_LFR.Gas(i,4,idxNew))...
            ...
            elseif j == 3
                ...
                Y_trainset = Mat_LFR_gasreduced.set3(i,:)';
                Y_predset = cat(1,squeeze(Mat_LFR.Gas(i,8,idxNew))...
            ...
            X_trainset = [sampling_AF.gas_ones; X_trainset];
            
            tblTrain_temp = X_trainset;
            tblTrain_temp(:,i) = [];
            
            tblTrain = array2table(...
                [X_trainset{:,i}, mean(tblTrain_temp{:,1:end},2)],...
                'VariableNames',{Equipment{i+2}, 'Other_Equip'});

            Y_trainset = [1; Y_trainset];
            tblTrain.Response = Y_trainset;

            mdl = fitlm(tblTrain,'quadratic','weights',weights,'Intercept',false);

            tblPred_temp = X_predset;
            tblPred_temp(:,i) = [];

            tblPred = array2table(...
                [X_predset{:,i}, mean(tblPred_temp{:,1:end},2)],...
                'VariableNames',{Equipment{i+2}, 'Other_Equip'});

            ypred = predict(mdl,tblPred);    
            ...
            axes(ha(j));
            h1 = scatter(Y_predset, ypred,4,'filled','MarkerFaceColor',ColorMat(i,:),'MarkerFaceAlpha',0.25);

            mdlfit = fitlm(Y_predset, ypred);
        ...
        txtbox = annotation('textbox',[0.37 0.03 0.61 0.24],...
                'string',sprintf('%s \nTrain n = %d\nTest n = %d',...
                Equipment{i+2},length(Y_trainset),length(Y_predset)),...
                'BackgroundColor','w');
    ...
    for i = 1:7
        ...
        for j = 1:3
            ...
            if j == 1
                ...
                X_trainset = repmat(sampling_AF.oil(idxTrain,:),3,1);
                X_predset = repmat(sampling_AF.oil(idxNew,:),3,1);                                   
                
                Y_trainset = Mat_LFR_oilreduced.set1(i,:)';
                Y_predset = cat(1,squeeze(Mat_LFR.Oil(i,1,idxNew))...
            ...
            elseif j == 2
                ...
                X_trainset = repmat(sampling_AF.oil(idxTrain,:),4,1);                   
                X_predset = repmat(sampling_AF.oil(idxNew,:),4,1);

                Y_trainset = Mat_LFR_oilreduced.set2(i,:)';
               Y_predset = cat(1,squeeze(Mat_LFR.Oil(i,4,idxNew))...
            ...
            elseif j == 3
                ...
                Y_trainset = Mat_LFR_oilreduced.set3(i,:)';
                Y_predset = cat(1,squeeze(Mat_LFR.Oil(i,8,idxNew))...
            ...
            X_trainset = [sampling_AF.oil_ones; X_trainset];
            ...
            tblTrain = array2table(...
                [X_trainset{:,i}, mean(tblTrain_temp{:,1:end},2)],...
                'VariableNames',{Equipment{equip_index(i)}, 'Other_Equip'});
        ...
        txtbox = annotation('textbox',[0.37 0.03 0.61 0.24],...
                'string',sprintf('%s \nTrain n = %d\nTest n = %d',...
                Equipment{equip_index(i)},length(Y_trainset),length(Y_predset)),...
                'BackgroundColor','w');
