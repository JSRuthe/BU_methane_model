...
% Lyon on August 14, 2019, with offshore and inactive wells removed
    ...
    filepath = fullfile(pwd, 'DrillingInfo/',csvFileName);
    file = fopen(filepath);
...
[M_woffshore,count,totalprod,averageprod] = data_class(M.new(:,1:2), cutoff, activityfolder);
...
M.raw = M.raw(M.raw(:,4) == 0,:);

[M_no_offshore,count,totalprod,averageprod] = data_class(M.new(:,1:2), cutoff, activityfolder);
...
%% GAS - 2 TRANCHE ANALYSIS

%(ii) Determine bin indices and bin means

    edges_gas_set = [0, 1, 5, 10, 20, 50, 100, 500, 1000, 10000,1500000];
    [counts_gas_all, edges_gas_all, ind_gas_all] = histcounts(M_no_offshore.gasall(:,2), edges_gas_set);
    
% %(iii) Determine bin means

    bin_ave_gasall = accumarray(ind_gas_all, M_no_offshore.gasall(:,2),[],@mean);
    bin_sum_gasall = accumarray(ind_gas_all, M_no_offshore.gasall(:,2),[],@sum);
    bin_sum_oilwg = accumarray(ind_gas_all, M_no_offshore.gasall(:,1),[],@sum);

    bins_exp = zeros((length(edges_gas_set)-1),4);
    
    bins_exp(:,1) = counts_gas_all';
    bins_exp(:,2) = bin_ave_gasall';
    bins_exp(:,3) = bin_sum_gasall';
    bins_exp(:,4) = bin_sum_oilwg';
    
%(iii) Print outputs to a text file
    
csvwrite('gas_bins_v2015_2tranche.csv',bins_exp)
      
%% OIL - 2 TRANCHE ANALYSIS

%(ii) Determine bin indices and bin means

    edges_oil_set = [0, 1, 5, 10, 20, 50, 100, 500, 1000, 10000,1500000];
    [counts_oil_all, edges_oil_all, ind_oil_all] = histcounts(M_no_offshore.oilall(:,2), edges_oil_set);
    
% %(iii) Determine bin means

    bin_ave_oilall = accumarray(ind_oil_all, M_no_offshore.oilall(:,2),[],@mean);
    bin_sum_oilall = accumarray(ind_oil_all, M_no_offshore.oilall(:,2),[],@sum);
    bin_sum_oilwg = accumarray(ind_oil_all, M_no_offshore.oilall(:,1),[],@sum);

    bins_exp = zeros((length(edges_oil_set)-1),4);
    
    bins_exp(:,1) = counts_oil_all';
    bins_exp(:,2) = bin_ave_oilall';
    bins_exp(:,3) = bin_sum_oilall';
    bins_exp(:,4) = bin_sum_oilwg';
    
%(iii) Print outputs to a text file
    
csvwrite('oil_bins_v2015_2tranche.csv',bins_exp)
...
hold on
    ...
    [~,edges] = histcounts(log10(M_no_offshore.gasall(:,2)));
    histogram(M_no_offshore.gasall(:,2),10.^edges,'Normalization','probability','DisplayStyle','stairs','LineStyle','-','LineWidth',3,'EdgeColor',StanfordRed);

    ylim([0 0.05]) 
    set(gca,'xscale','log')
    set(gca,'FontSize',8)
    set(gca,'XTick',[10^-2 10^0 10^2 10^4 10^6]);
    set(gca,'XTickLabel',{'10^{-2}', '10^{0}', '10^{2}', '10^{4}', '10^{6}'});
    xlabel('Production per well [mscf/well/day]');
...
hold on
    ...
    edges_gas_set = [0.001, 1, 5, 10, 20, 50, 100, 500, 1000, 10000,500000000];
    histogram(M_no_offshore.gasall(:,2),edges_gas_set,'Normalization','probability','FaceColor',StanfordRed,'FaceAlpha',0.5);
...
hold on
    ...
    [~,edges] = histcounts(log10(M_no_offshore.oilall(:,2)));
    histogram(M_no_offshore.oilall(:,2),10.^edges,'Normalization','probability','DisplayStyle','stairs','LineStyle','-','LineWidth',3,'EdgeColor',StanfordRed);
...
hold on
    ...
    edges_oil_set = [0.001, 1, 5, 10, 20, 50, 100, 500, 1000, 10000,500000000];
    histogram(M_no_offshore.oilall(:,2),edges_oil_set,'Normalization','probability','FaceColor',StanfordRed,'FaceAlpha',0.5);
...
print('-painters','-dmeta','Prod_Tranche_fig_22731.emf');
